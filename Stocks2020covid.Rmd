...
pprices %>%
  ...
  ggplot(.,aes(x=date,y=adjusted,colour=symbol, label=label)) +
...
```
Most recent stock price (`r format(Sys.time(), "%d %B, %Y")`) in relation to possible beginning recession fear time span highpoint. E.g. Abeona lost around 40% of its price until now.
